...
enum Api {
    ...
    private var package : ApiPackage {
        ...
        let uncompletedUrl                                      = NetworkDefaults.SERVER_ADDRESS
        
        switch self {
        case .Register(let username, let password, let code):
            return ApiPackage(
                url: uncompletedUrl + NetworkDefaults.SERVER_ROUTE + "?act=member&op=register&access_token=token",
                params: [
        ...
        case .Login(let username, let password):
            return ApiPackage(
                url: uncompletedUrl + NetworkDefaults.SERVER_ROUTE + "?act=member&op=login&access_token=token",
                params: [
        ...
        case .Logout(let userID, let token):
            return ApiPackage(
                url: uncompletedUrl + NetworkDefaults.SERVER_ROUTE + "?act=member&op=logout&access_token=token",
                params: [
        ...
        case .RequestPhoneValidatingCode(let phone):
            return ApiPackage(
                url: uncompletedUrl + NetworkDefaults.SERVER_ROUTE + "?access_token=token&act=member&op=getForgotCode",
                params: [
        ...
        case .RetrievePassword(let phone, let validateCode, let newPassword):
            return ApiPackage(
                url: uncompletedUrl + NetworkDefaults.SERVER_ROUTE + "?access_token=token&act=member&op=forgot",
                params: [
                    ApiParameterKey.MobilePhoneNumber           : phone,
        ...
        case .ChangePassword(let userID, let token, let password, let newPassword):
            return ApiPackage(
                url: uncompletedUrl + NetworkDefaults.SERVER_ROUTE + "?access_token=token&act=member&op=changepassword",
                params: [
        ...
        case .SubmitFeedback(let token, let content, let imageData):
            return ApiPackage(
                url: uncompletedUrl + NetworkDefaults.SERVER_ROUTE + "?act=member_feedback&op=member_feedback_add&access_token=token",
                params: [
                    ApiParameterKey.Token               : token,
                    ApiParameterKey.ClientType          : ApiParameterKey.DEVICE_TYPE_iOS,
